<template>
  <div class="index">
      <div v-if="message" class="relative w-[700px] mx-auto flex flex-col gap-6">
        <div class="text-bubble absolute top-20 -left-1/3 z-10">{{ message }}</div>
        <img :class="{'inverse': flash}" class="block mx-auto w-[700px]" src="https://images.prismic.io/zaneraywebsite/ff5b5f9c-5de2-40a4-9aac-2b91696747bf_team_henry.jpg" />
        <h2>Universal rendering with ssr</h2>
        <ul>
          <li>Initial request: ssr, in-app navigation: spa</li>
          <li>build time: builds all routes for server and client requests: .nuxt/dist/server and /client </li>
